...
public:
	...
	long Next(char c1)
	{
		if (pos<m_pos)
		{
			currhash = (currhash-ord(str[0]))/base + ord(c1)*p[curr_len-1];
			pos++;
		}
		else
		{
...
public:
	...
	unordered_map<long, vector<pair<int,int>>> hashtable;
...
//		Body[0].B.insert(pair<int,int>(0,1));
	...
	void GenPrimeTable(Node n1, int base, int len)
	{
		...
		long hash = 0;
		int S_len = n1.str.length();
		int pos = 0;
		
		if (S_len<len) //нода меньше хеша
		{
		...
		else //нода больше хеша
		{
			for (int i =0; i< len; i++)
			{
				GH.Inc(n1.str[i]);
			}
			AddHash(n1.ID,0,GH.currhash);
			pos = 0;

			for(int i=len; i<S_len; i++)
			{
				hash = GH.Next(n1.str[i-len],n1.str[i]);
				AddHash(n1.ID,i-len+1,hash);
				pos++;
			}
		}
		...
		{
			CallNode(n1.ID, *(node.second),n1.str, pos ,GH);
		}
	}

	void CallNode(int coreID, Node n1, string S, int pos, GenerHash GH2)
	{
		if (n1.ID==-1) return;
		int S_len = S.length()-pos;
		int N_len = n1.str.length();
		int L = min(S_len, N_len); 
		long lasthash = 0; 
		...
		if (GH2.formed)
		{
			for (int i =0; i<L; i++)
			{
				lasthash = GH2.Next(S[pos],n1.str[i]);
				AddHash(GH2.id,GH2.pos,lasthash);
				pos++;
			}		
		}
		else
		{
			...
			else
			{
				...
				AddHash(GH2.id,GH2.pos,GH2.currhash);
				
				if (S_len>1)
				{
				int L1 = min(S_len, N_len-y);

				for (int i =y; i<y+L1; i++)
				{
					lasthash = GH2.Next(S[pos],n1.str[i]);
					AddHash(GH2.id,GH2.pos,lasthash);
					pos++;
		...
		if (GH2.formed == false)
		{
			for (auto p1: n1.Next)
			{
				CallNode(coreID,*(p1.second),S, pos, GH2);
			}
		}
		else if ((S.length()>=n1.str.length())&&GH2.formed)
		{
			for (auto p1: n1.Next)
			{
				CallNode(coreID,*(p1.second),S, pos,GH2);
	...
	void AddHash(int ID, int pos, long hash)
	{
		unordered_map<long, vector<pair<int,int>>>::iterator it;
		...
		else
		{
			...
			hashtable.insert(pair<int,vector<pair<int,int>>>(hash,tmp));
		}
	}
	...
	{
		vector<long> hash_seeds;

		GenerHash GH2;

		GH2.Init(GH.base,GH.len);
		GH2.ReInit(0);
		for(int i =0; i<GH.len; i++)
		{
			GH2.Inc(str[i]);
		...
		for(int i =GH.len; i<str.length(); i++)
		{
			hash_seeds.push_back(GH2.Next(str[i-GH.len],str[i]));
		}
	...
	void AlignStepOne(vector<vector<pair<int,int>>> hit_place, vector<vector<PoAl>>  &map_blocks)
	{
		...
		for(int i=0; i<hit_place.size()-1; i++)
		{
			for(int j=0; j<hit_place[i].size(); j++)
			{
				AA1.Reinit(hit_place[i][j]);
				int q=1;
				while (AA1.Next(hit_place[i+q], (map_blocks[i])))
				{
					q++;
				}
			}
...
int main( int argc, const char* argv[] )
{
	...
	/*//string ref3 = loadfasta("C:/Users/Sergey/Documents/Visual Studio 2012/Projects/gPBWTAligner/Debug/seq1.fasta");

	string ref = "gccacgtgtgtgt";
	string ref2 = "tgtgtgtgcaccg";
	...
	clock_t t1 = clock();
	bool q = test.Search("atggccac",3);
	//test.ExactSearch("acgtgt");
	clock_t t2 = clock();
	...
	/*string tmp = test.substr(6,test.length()-6);
	...
	N_test1.Next.insert(pair<int,Node*>(2, &N_test2));
	N_test2.Prev.insert(pair<int,Node*>(1, &N_test1));

	tmp = test.substr(3,test.length()-3);
	...
	for(auto t1: N_test1.Next)
	{
		t1.second->Prev.erase(N_test1.ID);
		t1.second->Prev.insert(pair<int,Node*>(3,&N_test3));
		N_test3.Next.insert(t1);
	}

	N_test1.Next.clear();
	N_test1.Next.insert(pair<int,Node*>(3,&N_test3));*/
...
/*	Node N_test1(test,1);
	...
	N_test1.Split(6,&N_test2);

	N_test1.Split(3,&N_test3);
	

	system("pause");*/

	string test = "ACGTAACCGGTTAAACCCGGGTTT";
	//string test = "AAAAAAAAAAATTTTTTTTTTTTTTTT";
	Graph  testGraph;
	testGraph.LoadReference(test);
	...
	testGraph.NotSimpleFinderSNAP("AACCGGTTAAACCCGGGTT");
	system("pause");
